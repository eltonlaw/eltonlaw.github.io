...
`>RAM AT> FLASH` tells the linker to copy all of this from FLASH memory at startup. `.data` is non-zero initialized data. Global symbols `_sdata` and `_edata` are created to mark the start and end of this section. `.RamFunc` sections is code that is going to be executed from RAM rather than FLASH.
...
[`startup_stm32g070xx.s`](https://github.com/eltonlaw/grow_box/blob/ae2f98f6b05255ffc7b34f53357f5c7018cad453/src/startup_stm32g070xx.s) contains the code to run the setup necessary to execute the application `main`. The first lines are assembler directives.
...
The vector table is placed at `0x0000 0000` (of flash memory) and the pointer to the reset handler is defined to be at index 1.
...
3. Call `LoopCopyDataInit` which copies from r2
